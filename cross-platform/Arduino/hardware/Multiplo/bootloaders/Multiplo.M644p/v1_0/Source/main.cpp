...
/*
	...
	- ##PROBAR EL SISTEMA DE UMBRAL DE BATERÍA CON EL N10
		- Probar también el sistema de regulación del umbral desde el puerto, con el N10.
	- Se agregó que EL UMBRAL DE BATERÍA SEA CONFIGURABLE DESDE EL INTÉRPRETE:
		- Así el usuario puede personalizar esto desde XDF, o algo así, ya que antes de llamar al avrdude, el
		software puede configurar esto, quizá porque el el usuario informó de otro tipo de batería o placa de power.
	- Ver qué oc
	- DOCUMENTAR: Cuando se utiliza el módulo Comm.RS232, porque pareciera que no se recupera del autoreset y que quedara
	el pin de DTR en low, o algo así ¡¡MEDIR!!
		...
		puede seguirlo siendo en Processing, etc., cuando se usa el módulo Comm.RS232, puesto que en el circuito
		original de Arduino, la línea DTR está conectada a Reset por medio de un pequeño capacitor, para sólo
		resetear en el pulso inicial, cuando DTR cambia.
	- DOCUMENTAR: El sistema de execution flag (variable runAfterBootloading), sólo funciona si el software que abre
	el puerto para grabar al micro no vuelve a hacer un reset, por ejemplo, por culpa del avrdude. ##Ver si hay forma
	de hacer que el avrdude no envíe el pulso de reset al puerto.
	- Ver si se puede reducir aún un poco el tamaño del bootloader.
		- Quizá eliminando PrintInt y la libc, por ejemplo, quedemos a tiro para los 4096 bytes.
	- DOCUMENTAR: Valor de batería por USART:
		- Para que no interfiera con el avrdude, quizá lo mejor sea no enviar nada, y que el software tipo XDF y
		Minibloq ingresen al intérprete antes, al cual de todos modos deben ingresar para establecer si harán un
		download solamente, o un Run.
	- AVRDude + IDFS + Bootloader para Minibloq:
		- Quizá convenga reescribir el código de stk500, tomando el de avrdude, para manejar yo la apertura del puerto
		virtual, etc., para asegurar compatibilidad con nuestros módulos Bluetooth.
	- DOCUMENTAR: Autorun:
...
char serial0GetChar(void)
{
	...
	{
		/* ##DOCUMENTAR: Con botón de autorun, no arranca el bootloading:
			Este es el comportamiento esperado, porque de otro modo habría que añadir un time-out o algo
			así al Run, puesto que de entrar al bootloader y encontrar el Run button presionado, tendría que preguntar
			si hay data en el puerto, o algo así. Sin embargo, cuando la placa se usa en autorun, es probable que
			no esté conectada a nada que le pueda bajar software, ya que de estarlo, éste "master" podría enviar
			la orden de ejecutar por la UASRT. Esto así es, por ahora, una solución de compromiso. Si la placa
			entra al bootloader y está el jumper de Autorun colocado, entonces no se puede hacer bootloading, sino
		...
		*/
		...
		{
			/*
			##En realidad, la idea va a ser proteger de que no corte una escritura de página de la flash, no que no
			interrumpa la grabación general, porque no hay mucha forma de darse cuenta de que terminó de grabar,
			salvo que llegue un comando del avrdude (tipo 'Q', o que llegue un reset físico del entorno, o que el
			usuario resetee, en cuyo caso tampoco se puede proteger mucho nada, porque aborta a la mitad de la
			escritura), así que como esto está en el getchar, si presionan el Run button, y el bootloader lo lee, es
			porque está aquí intentando obtener nuevos caracteres de la USART (éste es el único lugar donde se lee
			el Run button):
			*/
			//##Parece que todo esto es encesaro para habilitar la sección RWW:
				/* Leave programming mode  */
				//nothing_response(); ##Esto no, porque sería una llamada recursiva!
		#ifdef WATCHDOG_MODS
				//serial0PrintStr("\r\nRun\r\n"); //##Debug.
			...
			else if(ch=='d')
			{
				...
				if (serial0GetChar() == ' ')
				{
					...
					else
					{
						...
						//##Ver si esto es afectado por el botón de Run:
